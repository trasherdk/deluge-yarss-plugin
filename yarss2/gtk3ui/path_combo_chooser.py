...
# -*- coding: utf-8 -*-
#
# Copyright (C) 2012-2019 bendikro bro.devel@gmail.com
#
...
def resource_filename3(package, filename):
    path = '%s/%s' % (os.path.abspath(os.path.dirname(os.path.dirname(sys.argv[0]))), filename)
    return path


def resource_filename2(package, filename):
    fn = get_resource(filename, path=None)
    # path = '%s/%s' % (os.path.abspath(os.path.dirname(os.path.dirname(sys.argv[0]))), filename)
    return fn
...
def is_scroll_down(event):
    """
    Check if scroll direction is down

    Args:
        event (Gdk.EventScroll): Mouse scroll event
    """
    is_down = None
    scroll_direction, direction = event.get_scroll_direction()
    if scroll_direction:
        is_down = event.direction == Gdk.ScrollDirection.DOWN,
    else:
        smooth_scroll_info, delta_x, delta_y = event.get_scroll_deltas()
        is_down = delta_y > 0
    return is_down
...
class PathChooserPopup(object):
    ...
    def set_window_position_and_size(self):
        if len(self.tree_store) < self.min_visible_rows:
            return False
        # Must show first so that self.treeview.get_preferred_height() returns proper values
    ...
    def get_position(self):
        ...
        # Get the requested treeview width
        col_size = self.treeview.get_column(0).cell_get_size()
        content_width = col_size.width + 10
        ...
        if len(self.tree_store) > 0 and self.max_visible_rows > 0:
            ...
            row_height = self.treeview.get_preferred_height()[1] / len(
                self.tree_store
            )
            # The preferred height is only valid on the first time the popup is made visible
            if row_height > 0:
...
class PathCompletionPopup(CompletionList, PathChooserPopup):
    ...
    def __init__(self, builder, path_entry, max_visible_rows):
        ...
        ] = self.on_completion_scroll_event
        self.signal_handlers[
    ...
    def on_completion_scroll_event(self, widget, event):
        """
        Handles scroll events from the treeview

        """
        _, x, y, modifier = event.window.get_pointer()
        self.handle_list_scroll(
            _next=is_scroll_down(event),
            set_entry=widget != self.treeview,
            scroll_window=True,
...
class PathChooserComboBox(Gtk.Box, StoredValuesPopup, GObject.GObject):
    ...
    def __init__(
        ...
        self.builder.add_from_file(
            resource_filename(
                __package__, os.path.join('data', 'path_combo_chooser.ui')
            )
        )
...
if __name__ == '__main__':
    ...
    for i, length in enumerate([100, 200], 2):
        label = Gtk.Label()
        label.set_text('%s' % length)
        label.set_size_request(length, 30)
        label.modify_bg(Gtk.StateFlags.NORMAL, Gdk.color_parse("green"))
